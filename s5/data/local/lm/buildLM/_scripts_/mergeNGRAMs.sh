...
general_domain_lm=zeroth.lm.tg.arpa.gz
task_domain_lm=corpus.lm.tg.arpa.gz
...
general_domain_lm=zeroth.lm.fg.arpa.gz
task_domain_lm=corpus.lm.fg.arpa.gz
test_corpus=corpus.test
if [ "$use_best" = true ]; then
	echo 'find the best mixing weight'
	ngram -lm $general_domain_lm -ppl $test_corpus -debug 2 > $test_corpus.ppl.general_domain
...
ngram -prune $prune_thresh_medium -lm $mixed_lm_fg -write-lm mixed_$lambda_fg.lm.fgmed.arpa.gz
echo '  4-gram medium: ' $(du -h mixed_$lambda_fg.lm.fgmed.arpa.gz)

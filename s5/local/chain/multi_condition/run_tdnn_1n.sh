...
tree_dir=test/models/korean/ebs_model_v1
...
if [ $stage -le 9 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" ${lores_train_data_dir} \
    data/lang_nosp $gmm_dir $clean_lat_dir
...
if $test_online_decoding && [ $stage -le 14 ]; then
 ...
 steps/online/nnet3/prepare_online_decoding.sh \
    ...
    $lang ${modeldir}/ivector_extractor ${modeldir} ${modeldir}_online
# steps/online/nnet3/prepare_online_decoding.sh \
#    --mfcc-config conf/mfcc_hires.conf \
    ...
    (
      ...
      for lmtype in tgsmall; do
        steps/online/nnet3/decode.sh \
          --acwt 1 --post-decode-acwt 10.0 \
          --nj 10 --cmd "$decode_cmd" \
	  --scoring_opts "--min_lmwt 5 --max_lmwt 10" \
	  $modeldir data/$data ${modeldir}/decode_${lmtype}_test_${data_affix}
          #$tree_dir/graph_${lmtype} data/${data} ${dir}_online/decode_${lmtype}_test_${data_affix} || exit 1
      echo "$lmtype done"
      fglarge="lang_test_fglarge"
      done
      steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
        data/lang_test_tgsmall data/$fglarge \
       data/${data} ${modeldir}/decode_${lmtype}_test_${data_affix} ${modeldir}/decode_${fglarge}_test_${data_affix}  || exit 1
